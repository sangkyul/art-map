...
function thumbUrl(imageUrl, width) {
  ...
  // For en.wikipedia Special:FilePath (fair-use images — thumb.php is Commons-only)
  ...
  // For Wikimedia Commons: use thumb.php with properly decoded filename
  // Exception: thumb.php returns 400 for TIFF files — fall back to Special:FilePath for those
  const m = imageUrl.match(/Special:FilePath\/(.+)$/);
  if (m) {
    const raw = m[1].split('?')[0];
    if (/\.tiff?$/i.test(raw)) {
      return `https://commons.wikimedia.org/wiki/Special:FilePath/${raw}?width=${width}`;
    }
    const filename = decodeURIComponent(raw).replace(/ /g, '_');
    return `https://commons.wikimedia.org/w/thumb.php?width=${width}&f=${encodeURIComponent(filename)}`;
  }
  const base = imageUrl.replace(/^http:/, 'https:');
  return base.includes('?') ? `${base}&width=${width}` : `${base}?width=${width}`;
}
...
function showClusterPopup(latlng, paintings) {
  const groups = {};
  paintings.forEach(p => {
    const key = locOf(p) || 'Unknown location';
...
document.querySelectorAll('.tab-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    document.getElementById('search-wrap').style.display = tab === 'map' ? '' : 'none';
