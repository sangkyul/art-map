...
function buildTimelineTab() {
  ...
  const withYear = PAINTINGS.filter(p => p.year != null && p.year >= BIN_START);
  const maxYear  = withYear.reduce((a, p) => Math.max(a, p.year), BIN_START + BIN_SIZE);
  const numBins  = Math.ceil((maxYear + 1 - BIN_START) / BIN_SIZE);

  // Bin arrays sorted by score descending
  ...
  // Top 10 paintings in the 1500+ range
  const top10 = withYear.slice().sort((a, b) => b.score - a.score).slice(0, 10);
  ...
  const cardItems = top10.map(p => {
    const binIdx = Math.min(numBins - 1, Math.max(0, Math.floor((p.year - BIN_START) / BIN_SIZE)));
    return { p, binIdx };
  ...
  function render(viewW) {
    const canvasW = Math.max(viewW, PAD_L + numBins * BIN_PX + PAD_R);
    ...
    laid.forEach(({ p, binIdx, rowIdx }) => {
      const binCx   = PAD_L + (binIdx + 0.5) * BIN_PX;
      // Clamp card position so it stays within canvas bounds
      const rawLeft = Math.round(binCx - W_CARD / 2);
      ...
      // Dot on axis
      ctx.beginPath();
      ctx.arc(binCx, axisY + 5, 3, 0, Math.PI * 2);
      ctx.fillStyle = '#666';
      ctx.fill();
